...
io.on('connection', function (socket) {
    ...
    socket.on('lottery', function (lottery) {
        ...
        if (parseInt(lottery.nBetList[0]) % len != 0) {
            socket.emit('lotteryResult', {ResultCode: -1});
            return;
        }
        var bet_num = parseInt(lottery.nBetList[0]) / len;
        var bet_list = [];
        for (var i = 0; i < len; i++) {
            bet_list.push(bet_num);
        }

        var result = gameInfo.lottery(socket.userId, lottery.bet, bet_list);
        if (result.code < 1) {
    ...
    //设置免费游戏模式
    socket.on('freeTimeType', function (Type) {
        if (!socket.userId) {
            return;
        }
        try {
        ...
        console.log(socket.userId, Type);
        let result = gameInfo.setFreeType(socket.userId, Type);
        if (result) {
            socket.emit('freeTimeTypeResult', {ResultCode: 0, ResultData: result});
        } else {
            socket.emit('freeTimeTypeResult', {ResultCode: -1, ResultData: null});
        }
    });

    //大奖旋转
    socket.on('bigWinGO', function () {
        if (!socket.userId) {
            return;
        }
        let result = gameInfo.playBigWin(socket.userId);
        if (result) {
            socket.emit('bigWinGOResult', {ResultCode: 1, ResultData: result});
        } else {
            socket.emit('bigWinGOResult', {ResultCode: -1, ResultData: null});
        }
    });
