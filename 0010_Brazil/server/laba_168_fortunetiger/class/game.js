    ...
    var Game = function () {
        ...
        this.lottery = function (_userId, _betIdx, _nBetList, isUse, coinCtrl) {
            ...
            while (true) {
                ...
                if (is_luck) {
                    //随机选一张牌
                    let chooseNum = RandomNumBoth(0, gameConfig.GAME_COLORS_DIAMOND.length - 1);
                    if (chooseNum === gameConfig.GAME_MAGIC_CARD_DIAMOND) {
                        ...
                        let finalList = [];
                        let startList = [];
                        nHandCards = [7, 7, 7, 7, 7, 7, 7, 7, 7];
                        for (let i = 0; i < startNum; i++) {
                            let a = RandomNumBoth(0, nHandCards.length - 1);
                            if (nHandCards[a] !== chooseNum) {
                ...
                for (let i = 1; i < nHandCards.length; i++) {
                    if (nHandCards[i] === nHandCards[i - 1] || nHandCards[i] === gameConfig.GAME_MAGIC_CARD_DIAMOND) {
                        sameNum += 1;
                    }
                }
                ...
                if (isUse === 1) {
                    if (score_before < coinCtrl) {
                        if (dictAnalyseResult["getOpenBox"]["bFlag"]) {
                            ...
                            if (parseInt(dictAnalyseResult["win"]) * parseInt(dictAnalyseResult["fMultiple"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) - nBetSum >= 0) {
                                break_check = true;
                            }
                        }
                    } else {
                        ...
                        if (dictAnalyseResult["getOpenBox"]["bFlag"]) {
                            ...
                            if (r === 100) {
                                break_check = true;
                            } else if (parseInt(dictAnalyseResult["win"]) * parseInt(dictAnalyseResult["fMultiple"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) - nBetSum <= 0) {
                                break_check = true;
                            }
