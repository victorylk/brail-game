            ...
            while (true) {
                ...
                if (!bFreeTimeFlag) {
                    ...
                    nHandCards = laba.CreateRandomCardList(gameConfig.Free_GAME_COLORS_DIAMOND,
                        gameConfig.GAME_HAND_CARDS_Column,
                        gameConfig.GAME_HAND_CARDS_FREE_Row,
                        gameConfig.GAME_HAND_CARDS_No_Magic_Column_List)
                }
                ...
                if (nHandCards[0] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[5] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[10] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[15] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[20] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[25] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[30] === gameConfig.GAME_MAGIC_CARD_DIAMOND ||
                    nHandCards[35] === gameConfig.GAME_MAGIC_CARD_DIAMOND) {
                    continue;
                }
                ...
                //随机生成变化牌
                let list = [];
                let maskPosition = [];//记录原始数组
                let oldHandCards = nHandCards.concat();
                if (bFreeTimeFlag) {
                    //免费模式中开场隐藏前4行，用特殊号码来代替未中奖
                    for (let i = gameConfig.GAME_HAND_CARDS_Column * gameConfig.GAME_HAND_CARDS_Row - 1; i >= 0; i--) {
                        if (nHandCards[i + 5] !== gameConfig.GAME_FREE_TIMES_CARD_DIAMOND && Math.random() > 0.5) {

                        } else {
                            nHandCards[i] = gameConfig.GAME_FREE_TIMES_CARD_DIAMOND;
                            maskPosition.push(i);
                        }
                    }
                } else if (Math.random() > 0.5) {//有一半概率触发
                    let idx = RandomNumBoth(0, 10);
                    let num = RandomNumBoth(1, 5);
                    list = randomDiffNum(gameConfig.GAME_HAND_CARDS_Column * gameConfig.GAME_HAND_CARDS_Row, num);
                    for (let i = 0; i < list.length; i++) {
                        nHandCards[list[i]] = idx;
                    }
                }
                ...
                //# 获取处理结果
                dictAnalyseResult = laba.AnalyseColumnSolt2(nHandCards,
                ...
                if (!bFreeTimeFlag && list_one_count(gameConfig.GAME_FREE_TIMES_CARD_DIAMOND, nHandCards) >= 3) {
                    ...
                    switch (t) {
                        case 3:
                            getFreeTime = {"bFlag": true, "nFreeTime": 5};
                            break;
                        case 4:
                            getFreeTime = {"bFlag": true, "nFreeTime": 7};
                            break;
                        case 5:
                            getFreeTime = {"bFlag": true, "nFreeTime": 10};
                            break;
                    }
                }
                if (bFreeTimeFlag) {
                    nHandCards = oldHandCards.concat();
                    dictAnalyseResult["maskPosition"] = maskPosition;
                }
                dictAnalyseResult["mysteryList"] = list;
                ...
                while (true) {
                    if (dictAnalyseResult["win"] > 0) { // 中奖了
                        ...
                        if (!bFreeTimeFlag) {
                            ...
                            for (let n_h_i in nHandCards) {
                                if (nHandCards[n_h_i] === -1) {
                                    if (parseInt(n_h_i) + 5 < 40 && nHandCards[parseInt(n_h_i) + 5] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 5];
                                        nHandCards[parseInt(n_h_i) + 5] = -1;
                                    } else if (parseInt(n_h_i) + 10 < 40 && nHandCards[parseInt(n_h_i) + 10] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 10];
                                        nHandCards[parseInt(n_h_i) + 10] = -1;
                                    } else if (parseInt(n_h_i) + 15 < 40 && nHandCards[parseInt(n_h_i) + 15] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 15];
                                        nHandCards[parseInt(n_h_i) + 15] = -1;
                                    } else if (parseInt(n_h_i) + 20 < 40 && nHandCards[parseInt(n_h_i) + 20] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 20];
                                        nHandCards[parseInt(n_h_i) + 20] = -1;
                                    } else if (parseInt(n_h_i) + 25 < 40 && nHandCards[parseInt(n_h_i) + 25] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 25];
                                        nHandCards[parseInt(n_h_i) + 25] = -1;
                                    } else if (parseInt(n_h_i) + 30 < 40 && nHandCards[parseInt(n_h_i) + 30] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 30];
                                        nHandCards[parseInt(n_h_i) + 30] = -1;
                                    } else if (parseInt(n_h_i) + 35 < 40 && nHandCards[parseInt(n_h_i) + 35] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 35];
                                        nHandCards[parseInt(n_h_i) + 35] = -1;
                                    } else {
                                        nHandCards[n_h_i] = gameConfig.Free_GAME_COLORS_DIAMOND[RandomNumBoth(0, gameConfig.Free_GAME_COLORS_DIAMOND.length - 1)];
                                    }
                                }
                        ...
                        if (bFreeTimeFlag) {
                            oldHandCards = nHandCards.concat();
                            for (let i = 0; i < maskPosition.length; i++) {
                                nHandCards[maskPosition[i]] = gameConfig.GAME_FREE_TIMES_CARD_DIAMOND;
                            }
                        }
