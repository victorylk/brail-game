...
Csocket.on('connected', function (msg) {
    ...
    var info = {serverName: "拉霸游戏_271_麻将胡了2pg", serverId: gameConfig.serverId, signCode: "slel3@lsl334xx,deka"};
    ...
    socket.on('lottery', function (lottery) {
        ...
        if (http_bc) {
            ...
            http_bc.getUserCtrl(data, (data) => {
                ...
                var result = gameInfo.lottery(socket.userId, lottery.bet, lottery.nBetList, isUse, coinCtrl);
                if (result.code < 1) {
                    ...
                    socket.emit('lotteryResult', {
                        ...
                        ResultData: {
                            ...
                            getFreeTime: result.getFreeTime,
                        }
                    });
                }
            });
...
console.log("拉霸游戏_271_麻将胡了2pg服务器启动");
