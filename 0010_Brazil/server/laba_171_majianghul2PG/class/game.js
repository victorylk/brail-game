    ...
    var Game = function () {
        ...
        this.lottery = function (_userId, _betIdx, _nBetList, isUse, coinCtrl) {
            ...
            while (true) {
                ...
                //添加
                //免费牌每列只许出现一张
                let col1 = [nHandCards[0], nHandCards[5], nHandCards[10], nHandCards[15], nHandCards[20]];
                let col2 = [nHandCards[1], nHandCards[6], nHandCards[11], nHandCards[16], nHandCards[21]];
                let col3 = [nHandCards[2], nHandCards[7], nHandCards[12], nHandCards[17], nHandCards[22]];
                let col4 = [nHandCards[3], nHandCards[8], nHandCards[13], nHandCards[18], nHandCards[23]];
                let col5 = [nHandCards[4], nHandCards[9], nHandCards[14], nHandCards[19], nHandCards[24]];
                ...
                while (true) {
                    if (dictAnalyseResult["win"] > 0) { // 中奖了
                        ...
                        if (!bFreeTimeFlag) {
                            for (let i = 0; i < lastGoldCards.length; i++) {
                                let n = 0;
                                if (19 - lastGoldCards[i] > 0 && nHandCards[19 - lastGoldCards[i]] === -1) {
                                    n++;
                                }
                            ...
                            for (let n_h_i in nHandCards) {
                                if (nHandCards[n_h_i] === -1) {
                                    if (parseInt(n_h_i) + 5 < 25 && nHandCards[parseInt(n_h_i) + 5] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 5];
                                        nHandCards[parseInt(n_h_i) + 5] = -1;
                                    } else if (parseInt(n_h_i) + 10 < 25 && nHandCards[parseInt(n_h_i) + 10] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 10];
                                        nHandCards[parseInt(n_h_i) + 10] = -1;
                                    } else if (parseInt(n_h_i) + 15 < 25 && nHandCards[parseInt(n_h_i) + 15] !== -1) {
                                        nHandCards[n_h_i] = nHandCards[parseInt(n_h_i) + 15];
                                        nHandCards[parseInt(n_h_i) + 15] = -1;
                                    } else if (parseInt(n_h_i) + 20 < 25 && nHandCards[parseInt(n_h_i) + 20] !== -1) {
