...
var GameInfo = function () {
    ...
    var Game = function () {
        ...
        this.init = function () {
            ...
            //读取rtp信息
            redis_dao.getRTP((value) => {
                if (value) {
                    this.lotteryTotal = value.lotteryTotal;
                    this.winTotal = value.winTotal;
                    this.curRTP = value.curRTP;
                }
            });
            ...
            schedule.scheduleJob(rule, function () {
                ...
                if (second % 5 === 0) {
                    if (self.lotteryTotal !== 0) {
                        let curRtp = Math.round(self.winTotal / self.lotteryTotal * 10000) / 100;
                        self.curRTP = curRtp;
                        redis_dao.updateRTP(curRtp, self.lotteryTotal, self.winTotal, (value) => {
                            // console.log(value);
                        });
                    }
                }
        ...
        this.lottery = function (_userId, _betIdx, _nBetList, isUse, coinCtrl) {
            ...
            //额定rtp
            let expectRTP = GamblingBalanceLevelBigWin.expectRTP;
            ...
            while (true) {
                ...
                if (!bFreeTimeFlag && list_one_count(gameConfig.GAME_FREE_TIMES_CARD_DIAMOND, nHandCards) >= 3) {
                    ...
                    dictAnalyseResult["win"] += nBetSum;
                    dictAnalyseResult["nWinDetail"].push(nBetSum);
                    dictAnalyseResult["nWinLines"].push(99);
                    for (let a in winline) {
                        dictAnalyseResult["nWinCards"][winline[a]] = true;
                    }

                    let i = Math.floor((Math.random() * gameConfig.FREE_GAME_TIMES.length));
                    let t = gameConfig.FREE_GAME_TIMES[i];
                    //3张即可触发免费游戏
                ...
                } else if (bFreeTimeFlag) {
                    dictAnalyseResult["win"] *= gameConfig.MUL_LIST[sourceFreeType];
                    for (let i in dictAnalyseResult["nWinDetail"]) {
                        dictAnalyseResult["nWinDetail"][i] *= gameConfig.MUL_LIST[sourceFreeType];
                    }
                }

                //统计特殊牌的数量
                let n = list_one_count(7, nHandCards);
                if (n >= 3) {
                    let winline = [];
                    for (let i = 0; i < nHandCards.length; i++) {
                        if (nHandCards[i] === 7) {
                    ...
                    dictAnalyseResult["nWinLinesDetail"].push(winline);
                    dictAnalyseResult["win"] += gameConfig.SCATTER_COMB[n - 3] * nBetSum;
                    dictAnalyseResult["nWinDetail"].push(gameConfig.SCATTER_COMB[n - 3] * nBetSum);
                    dictAnalyseResult["nWinLines"].push(99);
                    for (let a in winline) {
                        dictAnalyseResult["nWinCards"][winline[a]] = true;
                    }
                ...
                let newRtp = Math.round((this.winTotal + parseInt(dictAnalyseResult["win"])) / this.lotteryTotal * 10000) / 100;
                //rtp差额
                let rtpDif = newRtp - expectRTP;
                if (rtpDif > 10) {//超过太多不能开
                    if (parseInt(dictAnalyseResult["win"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) < nBetSum) {
                        break_check = true;
                    }
                } else if (rtpDif > -20) {//不做任何限制的区段
                    break_check = true;
                } else {//太低需要送分
                    if (this.curRTP - expectRTP > -20) {
                        break_check = true;
                    } else {
                        if (parseInt(dictAnalyseResult["win"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) > nBetSum * 5) {
                            break_check = true;
                        }
                    }
                }
                ...
                if (isUse === 1) {
                    if (score_before < coinCtrl) {
                        ...
                        } else if (dictAnalyseResult["getFreeTime"]["bFlag"]) {

                        } else {
                            if (parseInt(dictAnalyseResult["win"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) - nBetSum >= 0) {
                                break_check = true;
                            }
                        }
                    } else {
                        ...
                        if (dictAnalyseResult["getOpenBox"]["bFlag"]) {
                            ...
                            if (r === 100) {
                                break_check = true;
                            } else if (parseInt(dictAnalyseResult["win"]) + parseInt(dictAnalyseResult["getOpenBox"]["win"]) - nBetSum <= 0) {
                                break_check = true;
                            }
            ...
            if (freeCount > 0) {
                this.userList[_userId].SetFreeType(dictAnalyseResult["getFreeTime"]["nFreeType"]);
            }
