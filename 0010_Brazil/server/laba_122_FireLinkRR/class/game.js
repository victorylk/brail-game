        ...
        this.lottery = function (_userId, _betIdx, _nBetList) {
            ...
            while (true) {
                ...
                if (bFreeTimeFlag && sourceFreeCount > 0) {
                    ...
                    let hasMagic = false;
                    for (let i = 0; i < nHandCards.length; i++) {
                        if (nHandCards[i] == gameConfig.GAME_MAGIC_CARD_DIAMOND) {
                            hasMagic = true;
                        }
                    }
                    if (hasMagic && dictAnalyseResult["win"] > 0) {
                        let tempMul = RandomNumForList(gameConfig.GAME_FREE_MUL_DIAMOND[sourceFreeType]);
                        dictAnalyseResult["win"] += dictAnalyseResult["win"] * tempMul;
                        dictAnalyseResult["fMultiple"] = tempMul;
                    }
                }
                // 判断是否进入大奖模式
