        ...
        this.lottery = function (_userId, _betIdx, _nBetList, isUse, coinCtrl) {
            ...
            while (true) {
                ...
                for (let i = 0; i < nHandCards.length; i++) {
                    if (nHandCards[i] === 7) {
                        let m = RandomNumForList([0.5, 0.5, 0.5, 0.5, 0.5, 1, 1, 1, 1, 1, 5, 5, 5, 5, 5, 10, 10, 10, 10, 10, 50, 50, 50, 100, 100, 500]);
                        dictAnalyseResult["goldList"][i + ""] = m;
                    }
                }
                ...
                //判断是否中大奖
                if (list_one_count(7, nHandCards) >= 5) {
                    let mul = 0;
                    for (let i in dictAnalyseResult["goldList"]) {
                        mul += dictAnalyseResult["goldList"][i];
                    }
                    ...
                    dictAnalyseResult["win"] += mul * nBetSum;
                    dictAnalyseResult["nWinDetail"].push(mul * nBetSum);
