...
var api = function (req, callback) {
    ...
    if (req.query.act == "reg") {
    ...
    } else if (req.query.act == "diamondedit") {
        ...
        if (req.query.accountname && req.query.goldnum && req.query.time && req.query.sign) {
            ...
            //先通过帐号查询ID
            dao.getUserId(req.query.accountname, function (result, userid, userscore, p, diamond) {
                if (result) {
                    if (userid) {
                        if (req.query.goldnum > 0) {
                            var userInfo = {
        ...
        if (req.query.accountname && req.query.time && req.query.sign) {
            //先通过帐号查询ID
            dao.getUserId(req.query.accountname, function (result, userid, userscore, p, userdiamond) {
                if (result) {
                    ...
                    if (userItem) {
                        ...
                        sendStr = JSON.stringify({status: 0, msg: "", data: {score: userscore, diamond: userdiamond}});
    ...
    } else if (req.query.act == "isloginquery") {
        //判断用户是否在线

        //验证md5
        var content = req.query.act + req.query.user_id + req.query.time + key;
        var md5 = crypto.createHash('md5');
        md5.update(content);
        if (md5.digest('hex') != req.query.sign) {
            sendStr = '{"status":1,"msg":"参数不正确!"}'
        ...
        var userItem = gameInfo.getUser(req.query.user_id);
        if (userItem && userItem.Islogin()) {
            sendStr = '{"status":1,"msg":""}'
            callback(sendStr)
        } else {
            sendStr = '{"status":0,"msg":""}'
            callback(sendStr)
        }

    } else if (req.query.act == "saveAllUser") {
        //判断用户是否在线

        //验证md5
        var content = req.query.act + req.query.time + key;
        var md5 = crypto.createHash('md5');
        md5.update(content);
        if (md5.digest('hex') != req.query.sign) {
            sendStr = '{"status":1,"msg":"参数不正确!"}'
        ...
        gameInfo.saveAll();
        sendStr = '{"status":0,"msg":""}'
        // console.log("saveAllUser----------------------------------")
        callback(sendStr)

    } else if (req.query.act == "updateServerLog") {
        //判断用户是否在线
        ...
        dao.selectServerLog(function (success, result) {
            if (success) {
                sendStr = '{"status":0,"msg":""}';
                // console.log(result)
                gameInfo.server_log_list = result;
                gameInfo._io.sockets.emit('noticeMsg', result);
                callback(sendStr);
            } else {
                sendStr = '{"status":0,"msg":"!"}';
                gameInfo._io.sockets.emit('noticeMsg', []);
                callback(sendStr);
            }
            // console.log("server_log_list-------------------------------------------------------------")
            // console.log("server_log_list-------------------------------------------------------------")
            // console.log(gameInfo.server_log_list)
        })


    } else if (req.query.act == "queryOnlineNum") {
        //验证md5
        var content = req.query.act + req.query.time + key;
        var md5 = crypto.createHash('md5');
        md5.update(content);
        if (md5.digest('hex') != req.query.sign) {
            sendStr = '{"status":0,"msg":"参数不正确!"}'
            callback(sendStr);
            return;
        }
        callback(JSON.stringify({status: 1, result: {online_num: gameInfo.getOnlinePlayerCount()}}));
