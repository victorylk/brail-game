...
Csocket.on('connected', function (msg) {
    ...
    var info = {serverName: "拉霸游戏_302_luckyPoker", serverId: gameConfig.serverId, signCode: "slel3@lsl334xx,deka"};
...
io.on('connection', function (socket) {
    ...
    socket.on('lottery', function (lottery) {
        ...
        var result = gameInfo.lottery(socket.userId, lottery.bet, lottery.mul, lottery.type);
        if (result.code < 1) {
    ...
    //结算
    socket.on('settlement', function (info) {
        try {
            var data = JSON.parse(info);
            info = data;
        } catch (e) {
            //log.warn('settlement-json');
        }
        gameInfo.settlementGame(socket.userId, socket);
    });
