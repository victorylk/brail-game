...
cc.Class({
    ...
    onLoad() {
        this.audio = cc.director.getScene().getChildByName('Canvas').getComponent('ghostpiratesAudio');
        this.mainObj = cc.director.getScene().getChildByName('Canvas').getComponent('ghostpiratesMain');
            ...
            if (this.status == 1) {
                this.node.runAction(
                    cc.sequence(
                        ...
                        cc.moveTo(timer, cc.v2(this.node.x, -this.node.height + 520)),
                        cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 490)),
                        cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 520)),
    ...
    rollCallBack() {
        ...
        let scaNum = 0;
        if (this.roleIdList[this.roleIdList.length - 2] == 8) {
            scaNum += 1;
        }
        if (this.roleIdList[this.roleIdList.length - 3] == 8) {
            scaNum += 1;
        }
        if (this.roleIdList[this.roleIdList.length - 4] == 8) {
            scaNum += 1;
        }
        this.mainObj.setBigWinShow(scaNum);
    ...
    stopImmediately() {
        ...
        setTimeout(() => {
            this.node.y = -this.node.height + 520;
        }, 50);
        this.status = 0;
        let scaNum = 0;
        if (this.roleIdList[this.roleIdList.length - 2] == 8) {
