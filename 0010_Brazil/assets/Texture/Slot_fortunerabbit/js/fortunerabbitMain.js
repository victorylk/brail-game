...
cc.Class({
    ...
    onLoad() {
        ...
        this.net = this.node.getComponent('fortunerabbitNetwork');
    ...
    start() {
        ...
        //监听兔子动画特效
        this.rabbitAnim.getComponent(sp.Skeleton).setCompleteListener((event) => {
            switch (event.animation.name) {
                case "idle2":
                    this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "idle", true);
                    break;
                case "idle3":
                    this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "idle", true);
                    break;
        ...
        this.schedule(() => {
            ...
            let r = Math.random();
            if (r < 0.5) {
                this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "idle2", false);
            } else {
                this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "idle3", false);
            }
        }, 6);
    },
                    ...
                    for (let j = 0; j <= n; j++) {
                        if (this.wheelList[i].roleIdList[j + 1] == 7) {
                            ...
                            pb.runAction(cc.sequence(
                                ...
                                cc.jumpTo(0.3, cc.v2(-155, 464), 100, 1),
                                cc.removeSelf()
                            ))
    ...
    playWinAnim(freeTimeFlag) {
        ...
        if (this.lotteryRes.winscore > 0) {
            ...
            if (this.lotteryRes.winscore < this.betSum * 3) {
                this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "win", false);
            } else {
                this.rabbitAnim.getComponent(sp.Skeleton).setAnimation(0, "win2", false);
            }
        } else {
