...
cc.Class({
    ...
    onLoad() {
        ...
        this.net = this.node.getComponent('fortunetigerNetwork');
    ...
    start() {
        ...
        this.tigerAnim.getComponent(sp.Skeleton).setCompleteListener((event) => {
            switch (event.animation.name) {
                ...
                case "wild_collect":
                    ...
                    break;
                case "zo_start":
                    this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "zo_idle", true);
                    break;
                case "zo_exit":
                    this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "idle", true);
                    break;
                case "rs_win":
                    this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "rs_win_idle", true);
                    break;
                case "rs_win_exit":
                    this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "idle", true);
                    break;
        ...
        this.schedule(() => {
            ...
            let r = Math.random();
            if (r < 0.5) {
                this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "idle4", false);
            } else {
                this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "idle5", false);
            }
        }, 5);
    },
    ...
    roll(list) {
        ...
        //判断是否进大奖
        if (this.lotteryRes.viewarray.getBigWin.bFlag) {
            let bigWinResult = this.lotteryRes.viewarray.getBigWin;
            this.bIsFreeGame = true;
            this.scheduleOnce(() => {
                this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "zo_start", false);
                if (bigWinResult.isStart) {
                    cc.find("Canvas/bg").runAction(cc.sequence(
                        cc.delayTime(0.3),
                        cc.scaleTo(0.5, 0.9),
                        cc.callFunc(() => {
                            this.RedBagEffect.active = true;
                            this.RedBagEffect.getComponent(cc.Animation).play();
                        }),
                        cc.delayTime(0.4),
                        cc.callFunc(() => {
                            let pb = cc.instantiate(this.rolePb[bigWinResult.card]);
                            pb.runAction(cc.sequence(cc.moveBy(0.5, 0, 200), cc.moveBy(0.4, 0, -400), cc.removeSelf()));
                            this.RedBagEffect.addChild(pb);
                        }),
                        cc.delayTime(1),
                        cc.callFunc(() => {
                            this.RedBagEffect.active = false;
                            this.X10Effect.active = true;
                            this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "rs_idle", true);
                        }),
                    ));
                } else {
                    cc.find("Canvas/bg").runAction(cc.sequence(
                        ...
                        cc.scaleTo(0.5, 0.9),
                        cc.callFunc(() => {
                            this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "zo_exit", false);
                        }),
                        cc.delayTime(0.8),
                        cc.scaleTo(0.5, 1),
                        cc.callFunc(() => {
            ...
            //免费模式是开场随意转
            for (let i in this.wheelList) {
                this.wheelList[i].startFreeRoll(...line[i]);
            }
        } else {
            for (let i in this.wheelList) {
    ...
    showBigWin() {
        this.slotCtrl.setSpinAnim(3);
        for (let i in this.wheelList) {
            this.wheelList[i].removeWheel();
        }
        let bigWinResult = this.lotteryRes.viewarray.getBigWin;
        let list = bigWinResult.list[0];
        this.effectCard.scale = 0.9;
        for (let j in list) {
            let cols = list[j] % 3;
            let index = parseInt(list[j] / 3);
            let effectCardList = this.effectCard.children;
            for (let i = 0; i < effectCardList[cols * 3 + index].children.length; i++) {
                effectCardList[cols * 3 + index].children[i].active = i == (bigWinResult.card - 1);
            }
        }
        for (let i = 0; i < this.lotteryRes.viewarray.nWinLines.length; i++) {
            this.effectLine.children[this.lotteryRes.viewarray.nWinLines[i]].active = true;
        ...
        this.scheduleOnce(() => {
            ...
            this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "rs_win", false);
            if (this.lotteryRes.winscore > 0) {
                let wl = this.winNode.getChildByName("lab").getComponent(cc.Label);
                //判断播放金币掉落动画
                if (this.lotteryRes.winscore > this.betSum * 15) { //如果大于80倍赌注，就播放bigFull动画
            ...
            this.scheduleOnce(() => {
                cc.find("Canvas/bg").runAction(cc.sequence(
                    cc.scaleTo(0.5, 1),
                    cc.callFunc(() => {
                        this.tigerAnim.getComponent(sp.Skeleton).setAnimation(0, "rs_win_exit", false);
                        this.bIsFreeGame = false;
                        this.effectCard.scale = 1;
                        this.slotCtrl.setSpinAnim(0);
                        this.tipNode.active = true;
                        this.winNode.active = false;
                        this.win2BG.active = false;
