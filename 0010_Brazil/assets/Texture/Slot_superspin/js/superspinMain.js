...
cc.Class({
    ...
    onLoad() {
        ...
        this.net = this.node.getComponent('superspinNetwork');
        this.audio = this.node.getComponent('superspinAudio');
        this.wheelList = [];
        this.bet = 0;
        this.auto = false;
        this.status = 0;
    ...
    onCLick(event, args) {
        ...
        switch (args) {
            ...
            case 'add':
                ...
                this.bet += 1;
                this.bet = this.bet >= this.betList.length ? this.betList.length - 1 : this.bet;
                this.setBet();
                break;
            case 'dec':
                if (this.freeTimes > 0 || this.bigWinBoo || this.stopFree || this.auto) {
            ...
            case 'max':
                ...
                this.bet = this.betList.length - 1;
                this.setBet();
                break;
            case 'setting':
    ...
    setBet() {
        ...
        for (let i = 0; i < this.lblPrice.children.length; i++) {
            if (this.priceList[i] != 0) {
                this.lblPrice.children[i].getComponent(cc.Label).string = Helper.fixNum(this.priceList[i] * this.betList[this.bet]);
            }
        }
    },
    //设置按钮变化
    ...
    playWinAnim() {
        ...
        if (this.lotteryRes.winscore > 0) {
            ...
            cc.tween(this.effectAnimFullB)
                .delay(2)
                .to(0.5, { scale: 0.6, position: cc.v2(0, -270) })
                .call(() => {
