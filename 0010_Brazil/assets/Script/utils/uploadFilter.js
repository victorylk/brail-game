(() => {
  let uploadFilter = function () { };

  uploadFilter.prototype.createInput = () => {
    let newInput = document.createElement('input');
    newInput.type = "file";
    newInput.accept = "image/png";
    newInput.id = "file";
    newInput.style = "opacity:0;position:absolute;left:-100px;width:1px;height:1px;"
    newInput.onchange = window.webBase64String;
    document.body.appendChild(newInput);
  };

  if (typeof (window) != "undefined") {
    window.UploadFilter = Object.create(uploadFilter.prototype);

    window.webBase64String = function (e) {
      let param = e.target;
      ...
      reader.onload = function (event) {
        ...
        image.onload = function () {
          cc.log(fileList.size)//由于不能将太多Base64字符给服务端发过于，咱们压缩一下//如果想支持更大图片，请继续加判断，增加除数
          ...
          // cc.log(dataImg);//此时的dataImg就是你要上传给服务器的字符
          cc.find("Canvas").getComponent("LobbyMain").chatImg = dataImg;
          cc.find("Canvas").getComponent("LobbyMain").netWork.socket.emit("sendImgToUser", {
            userId: cc.find("Canvas").getComponent("LobbyMain").chatUserId,
            files: dataImg
          })
        }
      };
